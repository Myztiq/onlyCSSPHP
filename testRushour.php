		...
		<style>
			...
			<?
				...
				for($i=1;$i<count($cars);$i++){
					$activateTime = $cars[$i]->activateTime*.7;
					if( $cars[$i]->phase == 1){
						$activateTime+=1;
					}

					print ".gameBoard:hover #car".$cars[$i]->getID()."{
						-webkit-animation: animation".$cars[$i]->getID()." ".$cars[$i]->maxLength."s ".($activateTime)."s linear;
					}
...
<?PHP
	...
	class car{
		...
		function __construct($activateTime,$isWinningCar=false,$phase) {
...
//			$this->maxCars = 5;
		...
		function addCars(){
			...
			while(count($freeSpots) > $this->width-$this->getMaxCars()){
				...
				$car = new car($this->time,false,$this->phase);
